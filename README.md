...
后台编辑器可通过 `POST /v1/ai/polish` 请求 AI 润色候选。该接口只允许 `role = admin` 的后台 JWT 调用，默认在未配置 AI 提供商、Base URL、API Key 或模型时返回未配置错误。站点 owner 可在 `/backend` 的 AI 提供商页面实时修改配置；保存后的 API Key 会加密入库，接口响应只返回“是否已配置”。AI 润色只返回候选文本，不会自动保存或发布文章；应用候选后仍需手动点击“保存文章”。

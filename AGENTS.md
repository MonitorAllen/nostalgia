...
Nostalgia 是一个个人博客系统，提供博客前台展示、后台内容管理、文件上传、任务队列、权限管理、邮件验证等能力。项目包含 Go 后端服务、gRPC/Gateway 接口、PostgreSQL/Redis 基础设施，以及两个 Vue 3 前端应用。

## 技术栈

- 后端：Go、Gin、gRPC、gRPC-Gateway、sqlc、golang-migrate、Swagger/statik。
- 数据：PostgreSQL，使用 PGroonga 支持搜索；Redis 用于缓存或异步任务相关能力。
- 前端：Vue 3、Vite、TypeScript；后台使用 Element Plus/Pinia，前台使用 Tailwind CSS、Reka UI、Pinia，并通过 Bun 安装依赖与构建。
...
- `web/frontend/`：博客前台 Vue 应用。
- `web/backend/`：后台管理 Vue 应用。
...
- 前台修改应至少运行 `cd web/frontend && bun run type-check && bun run build`。
- 后台修改应至少运行对应应用的类型检查和构建命令。
...
本文件依据 `README.md`、`Makefile`、`go.mod`、`web/frontend/package.json`、`web/backend/package.json`、`sqlc.yaml`、`.github/workflows/*.yml` 和项目目录结构整理。

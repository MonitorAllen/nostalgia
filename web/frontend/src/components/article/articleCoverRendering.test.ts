...
describe('article cover rendering contracts', () => {
  ...
  test('public article list presents covers as medium-emphasis reading cards', () => {
    ...
    expect(list).toContain('md:self-center')
  ...
  test('admin article list uses shared covers and previews from cover clicks', () => {
    ...
    expect(view).toContain('defaultArticleCoverSrc')
    expect(view).toContain(':src="articleCoverSrc(article)"')
    expect(view).toContain('v-if="!hasCustomArticleCover(article)"')
    expect(view).toContain('未设置封面')
    expect(view).toContain('@click="openArticlePreview(article)"')
    expect(view).toContain(":aria-label=\"`预览 ${article.title || '无标题文章'} 封面`\"")
    expect(view).toContain('grid-cols-[6.5rem_minmax(0,1fr)]')
    expect(view).toContain('aspect-[16/9] w-full')
    expect(view).toContain('class="pointer-events-none"')
    expect(view).not.toContain('fallback-src="/images/go.png"')
  ...
  test('admin cover panel exposes guidance, warnings, and multi-surface previews', () => {
    ...
    expect(panel).toContain('class="min-w-0"')
    expect(panel).toContain('min-w-20 shrink-0 whitespace-nowrap')
